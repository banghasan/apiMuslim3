...
export const parseSchedulePeriod = (period: string):
  ...
  if (!/^[0-9]{4}$/.test(yearPart ?? "")) return null;
...
export const createJadwalService = (config: AppConfig) => {
  ...
  const safeJadwalTimeZone = (value?: string | null) => {
    ...
    if (!input) return defaultTimeZone;
    try {
  ...
  const loadScheduleFile = async (
    ...
    if (useCache && scheduleCache.has(key)) {
      return scheduleCache.get(key)!;
    }
    if (useCache && schedulePending.has(key)) {
  ...
  return { safeJadwalTimeZone, getTodayPeriod, loadScheduleFile };
};
