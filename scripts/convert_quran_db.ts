...
try {
  await Deno.mkdir(new URL("../data/quran", import.meta.url), { recursive: true });
} catch (e) {
...
db.execute("CREATE INDEX IF NOT EXISTS idx_ayahs_surah_number ON ayahs(surah_number)");
db.execute("CREATE INDEX IF NOT EXISTS idx_ayahs_surah_ayah ON ayahs(surah_number, ayah_number)");
